...
def check_webserver(address, port, resource):
    #create http quarry string
    if not resource.startswith('/'):
        
        resource = '/' + resource
        request_string = 'GET %s HHTP/1.1\r\nHost: %s\r\n\r\n' % (resource, address)
    ...
    try:
        ...
        #it's enough to receive first 100 bytes
        rsp = s.recv(100)
        print 'Receive 100 bytes of HTTP response'
        print '|||%s|||' % rsp
...
if __name__ == '__main__':
    ...
    parser.add_option('-p', '--port', type='int', dest='port', default=80, help='PORT for webserver', metavar='PORT')
